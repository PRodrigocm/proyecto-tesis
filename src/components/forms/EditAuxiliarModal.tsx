...
export default function EditAuxiliarModal({ isOpen, onClose, onSuccess, auxiliar }: EditAuxiliarModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/usuarios/${auxiliar.idUsuario}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        alert(`Error: ${error.message || 'No se pudo actualizar el auxiliar'}`)
      }
    } catch (error) {
      console.error('Error updating auxiliar:', error)
      alert('Error al actualizar el auxiliar')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
        <div className="flex justify-between items-center pb-3">
          <h3 className="text-lg font-bold text-gray-900">Editar Auxiliar</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2 text-gray-900">{auxiliar.ie.nombre}</span>
              </div>
              <div>
                <span className="font-medium text-gray-600">Rol:</span>
                <span className="ml-2 text-gray-900">
                  {auxiliar.roles.map(r => r.rol.nombre).join(', ')}
                </span>
              </div>
              <div>
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            >
              {loading ? 'Actualizando...' : 'Actualizar Auxiliar'}
            </button>
          </div>
        </form>
      </div>
